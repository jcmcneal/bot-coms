...
| Role | Tool | When |
|---|---|---|
| Assigner | `team_assign` | Hermes only. Dispatch slice or mid-task question (async, no wait). Never from coding agent. |
| Any | `team_bus` | status, list, verdict, log, `write_doc` (TEAM.md / STANDING.md), verify handle |
| Worker | `team_inbox` | on assign doorbell |
| Worker | `bot_coms_ack` | after agent launch + status stamp (RUNNING = no wake) |
| Worker | `team_report` | after runner wake → doorbell return address (`from`) |
| Worker | `agent_screen` | product work only |
...
RUNNING does not ring. Agent EXIT → `bot-coms job-done` (sidecar +
...
4. Assignee: `team_inbox` → ONE `agent_screen` → `team_bus status` → `bot_coms_ack`
...
- Never launch verify agent job from a wake
...
- Product code: one agent session, investigate → plan → implement in a worktree (not main). Not ask-then-respawn.
- No synchronous `ask_team`. Agent questions are PAUSED: Hermes `team_assign`s async, then resumes the same session via `session_id`.

## Mid-task questions (agent)

If the coding agent needs peer guidance mid-slice:
...
4. When the answer arrives, resume the **same** session with `session_id` from the sidecar.
5. Unattended in-agent question prompts are auto-skipped — do not rely on them.
6. Escalate to Jason only after peer/fallback routes, and only for a real decision.

`team_assign` is Hermes-to-Hermes. The coding agent never calls it.

## References

- [`references/operating-loop.md`](references/operating-loop.md) — from/to addressing
- [`references/agent-backends.md`](references/agent-backends.md) — Cursor / Codex / Claude Code
