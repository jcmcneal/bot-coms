"""Messaging heartbeat seatbelt: poke on empty-To and stale-work recovery."""
import asyncio
import json
import threading
import time
...
def _group_send(store, body='anyone?'):
    group = store.groups('test:alice', 'Bruv', ['swe-id', 'designer-id'], 'swe-id', 'g-seatbelt')
...
def test_stale_heartbeat_with_pending_empty_to_recovers_via_seatbelt(root):
    set_mode(root, 'on')
    selector = StubSelector({'action': 'yield', 'reason': 'human'})
    runtime = Runtime()
    ...
    async def runner():
        backend = MessagingService(
            ...
            runtime,
            selector=selector,
            seatbelt_stale_seconds=0.05,
            seatbelt_check_interval=0.02,
        )
        await backend.start()
        try:
            _group_send(backend.store)
...
def test_seatbelt_disarms_when_idle(root):
    set_mode(root, 'on')
    selector = StubSelector({'action': 'yield', 'reason': 'human'})
    runtime = Runtime()
    ...
    async def runner():
        ...
        await backend.start()
        try:
            assert backend._seatbelt is not None
            assert not backend.store.has_pending_work()
            assert backend._seatbelt._task is None
            _group_send(backend.store)
            deadline = time.monotonic() + 2
            while backend.store.has_pending_work() and time.monotonic() < deadline:
                await asyncio.sleep(0.02)
            assert not backend.store.has_pending_work()
            backend._seatbelt.sync()
            deadline = time.monotonic() + 0.5
            while backend._seatbelt._task is not None and time.monotonic() < deadline:
